...
import { deleteSalesRepresentativeById, createSalesRepresentative } from 'apiSdk/sales-representatives';
...
function OrganizationViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'account_manager', 'end_customer', 'sales_representative', 'team_member'],
      }),
  );
  ...
  const account_managerHandleCreate = async () => {
    setCreateError(null);
    try {
      await createAccountManager({ organization_id: id, user_id: account_managerUserId });
  ...
  const end_customerHandleCreate = async () => {
    setCreateError(null);
    try {
      await createEndCustomer({ organization_id: id, user_id: end_customerUserId });
  ...
  const [sales_representativeUserId, setSales_representativeUserId] = useState(null);
  const sales_representativeHandleCreate = async () => {
    setCreateError(null);
    try {
      await createSalesRepresentative({ organization_id: id, user_id: sales_representativeUserId });
      setSales_representativeUserId(null);
      await mutate();
    } catch (error) {
      setCreateError(error);
    }
  };
  const sales_representativeHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              ...
              <Stack spacing={2} mb={8}>
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect
                      name={'sales_representative_user'}
                      value={sales_representativeUserId}
                      handleChange={setSales_representativeUserId}
                    />
                  </Box>
                  <Button
                    colorScheme="blue"
                    mt="4"
                    mr="4"
                    onClick={sales_representativeHandleCreate}
                    isDisabled={!sales_representativeUserId}
                  >
                    Create
                  </Button>
                </Flex>
                <TableContainer>
                  <Table variant="simple">
                    ...
                    <Tbody>
                      {data?.sales_representative?.map((record) => (
                        <Tr
                          cursor="pointer"
                          onClick={() => router.push(`/users/view/${record?.user?.id}`)}
                          key={record?.user?.id}
                          ...
                          <Td>
                            {hasAccess('user', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                              <IconButton
                                onClick={(e) => {
                                  e.stopPropagation();
                                  sales_representativeHandleDelete(record.id);
                                }}
                                colorScheme="red"
                                variant="outline"
                                aria-label="edit"
